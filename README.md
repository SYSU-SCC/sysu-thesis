...
这个模板根据2016年中山大学本科毕业论文Word模板设计的。本人2016级毕业生，毕业论文是根据这个LaTex模板编写的。该模板同时包含了一个用LaTex Beamer书写的用于答辩的PPT模板。模板中给出了论文书写中经常会用的一些代码例子：如插入图像，绘制表格，描述算法流程，书写公式等。同学们可以参考这些例子来满足自己的需求。

该模板只是定义了一些基本的宏，设置了简单的布局来方便同学们写毕业论文，并不是一个规范的LaTex模板。限于水平和时间，我只能大概的整理一下，给出一些基本的文档说明。
同学们可以先打开`main.pdf`和`presentation/pre.pdf`看看效果，如果合适的话再考虑使用。
...
0. 目录: 自动生成章节与图表目录，`sysuthesis`设置
0. 各个章节：引言，基础理论知识，方法，实验与结果，总结与展望 `docs/chap0x`
...
## 致谢
0. 在这里我要感谢@chungkwong师兄，他是第一个在Github上放出了中大的第一个[非官方的本科论文LaTex模板](https://github.com/chungkwong/sysu_thesis)，而本模板是在@chungkwong师兄的版本上改进的。
...
#### Topic: Combining LSTM and Convolutioanl Feature for Semantic Segmentation
